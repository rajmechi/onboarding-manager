...
LDFLAGS := "-X github.com/$(REPOSITORY_NAME)/version.Version=$(VERSION) \
	-X github.com/$(REPOSITORY_NAME)/version.Vcs=$(BUILD_COMMIT) \
	-X github.com/$(REPOSITORY_NAME)/version.Timestamp=$(BUILD_TIMESTAMP) \
	-X github.com/$(REPOSITORY_NAME)/version.Hostname=$(BUILD_HOSTNAME)"
...
manager: generate fmt vet
	go build -o build/_output/bin/$(PROJECT_NAME)  -ldflags $(LDFLAGS) github.com/$(REPOSITORY_NAME)/cmd/manager

# Build manager binary
manager-osx: generate fmt vet
	GOOS=darwin go build -o build/_output/bin/$(PROJECT_NAME) -ldflags $(LDFLAGS) github.com/$(REPOSITORY_NAME)/cmd/manager
